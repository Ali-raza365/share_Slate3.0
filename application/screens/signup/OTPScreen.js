...
const OTPScreen = ({navigation}) => {
    ...
    const _onValidateClick = async () => {
        if(state.signup_otp.length == 0)
            alert('OTP is required');
        else if(state.signup_otp.length != 6)
            ...
            .then((email)=>{
                let details = {
                    email: email,
                    otp: state.signup_otp,
                };
                dispatch(_validatingSignupOTP(details, navigation));
            })
            .catch(()=>{
    ...
    return(
        <View style={Styles._mainContainer}>
            ...
            <Loader 
                ...
                isVisible={state.signup_otploading}
            />
            {/* Loaders */}
            <Loader 
                label='Resending OTP...'
            ...
            >
                ...
                <OtpInputs
                    ref={otpRef}
                    handleChange={text => dispatch(_onSignupOTPChange(text))}
                    numberOfInputs={6}
...

{
                ...
                <Button 
                    ...
                    //onPress={()=>{ _gotoSetPassword(navigation) }}
                    title='Validate'
                    titleColor={COLORS.whiteColor}
                    backgroundColor={COLORS.primaryColor}
                    style={{ marginVertical : WP(SPACING_PERCENT) }}
                />

                {/* {
                    !time ? (
                        <Text style={Styles._resend}>Resend OTP</Text>
                    ):(
                        <CountdownCircleTimer
                            isPlaying
                            duration={10}
